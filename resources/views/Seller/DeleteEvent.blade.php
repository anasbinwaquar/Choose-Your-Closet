...
<nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0">
            <div class="container-fluid d-flex flex-column p-0">
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar" style="margin-top: 30%;">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="/DeleteEvent"><i class="fas fa-tachometer-alt"></i><span>Delete Event Sale</span></a></li>
                ...
                <nav class="navbar navbar-light navbar-expand bg-white shadow mb-4 topbar static-top">
                    <div class="container-fluid"><button class="btn btn-link d-md-none rounded-circle mr-3" id="sidebarToggleTop" type="button"><i class="fas fa-bars"></i></button>
                        <form class="form-inline d-none d-sm-inline-block mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
                                <h3 class="text-dark mb-2">Delete Event Sale</h3>
                          <!--   </div> -->
                        </form>
          ...
          <tbody>
            @foreach ($events as $events)
              <tr>
                ...
                <th>{{$events->EventName}}</th>
                <th>{{$events->Discount}}</th>
                <th>
                    @foreach($products as $e)
                    @if($e->EventID==$events->EventID)
                        Product ID: {{$e->id}}  
                        <img src="{{asset('uploads/sell/'. $e->product_image)}}" style="width: 100px;height: 100px;">
                        <br>
                    @endif
                    @endforeach
                </th>
                <th><a href="/DestroyEvent/{{$events->EventID}}"><button class="btn btn-danger">Delete</button></a></th>
              </tr>
            @endforeach
        </tbody>
    </table>
